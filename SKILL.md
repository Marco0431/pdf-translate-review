...
7. **三类已知假阳性，先排除再动手**（2026-09-13 全库复核的统计口径）：① 定位器对**密集多行公式与表格**常配错（配到插图或空白条带），这类条目的指标无意义——按声明槽位人工定位后再看；② 等式裁图若来自约 **135–150 dpi 的重采样**，细笔画变灰会触发 `INK_MISSING` / `GLYPH_VANISHED` / `SOLID_BLOB`，而"实心块"常是 `=` 的两横或大括号笔画——交付尺寸下仍可读就不算缺陷；③ 检查器给插图定义的是**条带**，会把相邻图的照片一并圈进去，于是把"复现了相邻图"报成 critical（实战：Zhao2024 图 9 被报成复现 Fig. 10，核实为误报）。判死只认三种：**缺笔画导致读数错误、内容与槽位编号不符、两幅图同图**。

### 5.2c 内嵌图“重影”核对（交付 HTML 时必跑）

**为什么单列一节**：5.2 的字节去重与 5.2b 的裁图保真都看不见它——重影是**同一段字被画了两遍、错开一两像素**（截图时页面被重绘或缩放两次），内容、编号、对位数全对，只有把每张内嵌图与原文 PDF 的同一区域做**墨迹记账**才会现形。实战一轮全库检查（24 篇、441 张内嵌图）据此修掉 93 张。
...
判据（用改前/改后两组真实样本标定，见 `scripts/check_image_ghosts.py` 顶部说明）：

1. **不能只用 `mad`（同尺度重建后的平均像素差）**：不是同一渲染器出的图（无头浏览器按缩放截的）`mad` 天然 2–17、`ink_iou` 却 0.9+，并排看并没有重影；反过来，**尺度集合里漏掉 0.25（= 600 dpi）会把一批高清图整批跳过**。尺度用 `med`（像素差中位数）来选：定位正确时它恒为 ~0，与渲染方式无关；定位错误时它很大。
2. **真重影三项特征**：`ink_ratio ≥ 1.03`（多出一层描边，可以到 2.6）、`unexplained_rel ≥ 0.02`（多出来的墨落在原文的空白处）、`vanished_rel ≈ 0`（只多不少；若是“定位差一两像素”或“另一套渲染器”，`vanished` 会与 `unexplained` 同量级）。
3. **定位闸门**：`med ≤ 2` 且 `ink_iou ≥ 0.30` 且定位区域内墨迹 ≥ 200 px，才算“判得下来”。三条不满足的记为 `unverified`——**既不算通过、也不算缺陷**，多为白底折线图或早期流水线的图（模板匹配会配到空白处）。实战 441 张里 147 张属这类，抽查 16 张目视全部干净，可直接看图（`embedded | 干净渲染 | 差值`）。
4. 修法只有一种：在原 PDF 里定位同一区域，**按 2 倍像素重裁**，并在 `style` 里写死 `width:原像素宽`——显示尺寸与版面一律不动。收尾复核：被换的图恰好 2 倍像素、是 PNG、`<img>` / `<table>` / `data:` 的个数一个不变，HTML 文字层（去掉 `---` 后）与改前**逐字相同**、PDF 全文 span 只少掉那些 `---`。
5. 重影与 `---` 常同时出现在同一批文件里；两者都要顺手清（见 5.3）。
